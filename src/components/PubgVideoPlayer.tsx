...
import { Play, Maximize2, ExternalLink, Video, Film, CheckCircle2, RotateCw } from 'lucide-react';
...
export const PubgVideoPlayer: React.FC<PubgVideoPlayerProps> = ({
  ...
  return (
    <div className={`relative w-full aspect-video bg-[#0c0e17] overflow-hidden rounded-t-3xl border-b border-white/10 group ${className}`}>
      {/* 1. Subtle, Minimal Header Badges (Clean, Non-Intrusive, Space-Efficient) */}
      <div className="absolute top-2.5 left-2.5 right-2.5 z-20 flex items-center justify-between pointer-events-none">
        {/* Right side: Level & Verified tag */}
        <div className="flex items-center gap-1.5 pointer-events-auto">
          {level && (
            <span className="px-2 py-0.5 rounded-lg bg-black/75 backdrop-blur-md text-white font-mono font-bold text-[10px] sm:text-xs border border-white/10 shadow-sm">
              {level}
            </span>
          )}
          {badge && (
        ...
        {/* Left side: Cinema / Fullscreen Expansion Action */}
        {hasVideo && onExpand && (
          <div className="flex items-center gap-1 pointer-events-auto">
            <button
              ...
              title="تكبير الفيديو شاشة كاملة"
              aria-label="تكبير الفيديو شاشة كاملة"
            >
